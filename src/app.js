...
const {
  helmetMiddleware, authLimiter, publicApiLimiter, writeLimiter, readLimiter, reportLimiter,
  ipBlockCheck, attackDetection, setSecurityEventWriter,
  ...
  .then(() => {
    setSecurityEventWriter(async (event) => {
      ...
      await pool.execute(
        ...
        [event.type, event.level, event.ip, (event.path || '').substring(0, 255), (event.detail || '').substring(0, 1000), event.blocked ? 1 : 0],
      )
    })
